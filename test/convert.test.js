...
test("발주서에 나가는 날짜는 시간이 없다 ('UTC 자정' 으로 맞춘다)", async () => {
  // ★ 로컬 자정이 아니라 UTC 자정이어야 한다.
  //   ExcelJS 는 Date → 엑셀 일련번호로 바꿀 때 UTC 를 기준으로 삼는다.
  //   로컬 자정으로 넣으면 한국(UTC+9)에서 일련번호가 0.375 만큼 모자라
  //   새벽 주문이 엑셀에서 전날로 보인다. (qo-logic.js 의 dateOnlyCell 참고)
  const out = await run([ROW1]);
  const c = out.worksheets[0].getRow(2).getCell(2);
  assert.ok(c.value instanceof Date);
  assert.strictEqual(c.value.getUTCFullYear(), 2026);
  assert.strictEqual(c.value.getUTCMonth(), 6);
  assert.strictEqual(c.value.getUTCDate(), 28);
  assert.strictEqual(c.value.getUTCHours(), 0, "시가 남아 있다");
  assert.strictEqual(c.value.getUTCMinutes(), 0, "분이 남아 있다");
  assert.strictEqual(c.value.getUTCSeconds(), 0, "초가 남아 있다");
});

test("★ 새벽 주문이 전날로 밀려 나가지 않는다", async () => {
  // 한국 새벽 1시 주문. 로컬 자정으로 넣으면 엑셀에서 7/27 로 보였다.
  const row = [...ROW1];
  row[3] = new Date(2026, 6, 28, 1, 20);
  const out = await run([row]);
  const c = out.worksheets[0].getRow(2).getCell(2);
  assert.strictEqual(c.value.getUTCDate(), 28, "날짜가 하루 밀렸다");
});
