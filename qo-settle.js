...
const ST = (() => {
  const FIELDS = [
    ...
    /* ★ 이름은 '읽을 때' 만든다. 여기서 문자열로 굳히면 이 파일이 로드되는 시점 —
       즉 로그인 전이라 이 배포본의 원래 회사(랩노마드) 이름이 박혀서,
       베타브릭스 화면에 '랩노마드 매출(개당)' 이 뜬다. */
    { k: "unitPrice", get n() { return ((typeof CONFIG !== "undefined" && CONFIG.company) || "우리") + " 매출(개당)"; },
